...
- 🔄 **Hoist the energy API + its closure into `:common`.** ✅ **api-core foundation hoisted:** `Action` (clean enum; its `IFluidHandler.FluidAction` convenience → NeoForge-side `mekanism.api.fluid.FluidActions`, 7 sites), `AutomationType`, `IContentsListener`, `SerializationConstants`, and `mekanism.api.annotations.*` (added `jsr305` `compileOnly` to `:common`). Build green; bundles into main/-all/-api + Fabric jars. **Remaining for energy:** hoist `mekanism.api.math` + `mekanism.api.container` (they cascade with energy), extract `MekanismAPI.getService` (loader-clean service-locator) into `:common` for `IEnergyConversionHelper`, then move the 6 `mekanism/api/energy/` files.
