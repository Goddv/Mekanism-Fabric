...
- [ ] **Abstract `ValueIOSerializable`** → a common interface (e.g. `mekanism.api.IValueIOSerializable` with `serialize(ValueOutput)`/`deserialize(ValueInput)`, vanilla types); make `IEnergyContainer` extend it; on NeoForge adapt to NeoForge's `ValueIOSerializable` where the platform expects it.
...
- [ ] **Hoist the energy API package** (`mekanism/api/energy/`, 6 files) into `:common`; verify `:common:compileJava` + `:neoforge` (consuming + bundling common) green.
