...
public class FormulaicCraftingSlot extends BasicInventorySlot {
    ...
    private FormulaicCraftingSlot(BooleanSupplier autoModeSupplier, @Nullable IContentsListener listener, int x, int y) {
        super(ConstantPredicatesBase.alwaysTrueBi(), (stack, automationType) -> automationType == AutomationType.INTERNAL || !autoModeSupplier.getAsBoolean(),
              ConstantPredicatesBase.alwaysTrue(), listener, x, y);
